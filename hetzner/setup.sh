...
log() {
  ...
  local discord_color=3447003

  case "$color" in
    red)    echo -e "\033[0;31m=> $msg\033[0m" >&2; discord_color=16711680 ;;
    green)  echo -e "\033[0;32m=> $msg\033[0m" ;;
    yellow) echo -e "\033[1;33m=> $msg\033[0m" ;;
  esac

  echo "$msg" > "$HEARTBEAT_FILE"
  server-notify "$msg" "$discord_color" >/dev/null 2>&1 || true
}
...
heartbeat() {
  while true; do
    sleep 30
    server-notify "Still working on: $(cat "$HEARTBEAT_FILE")..." >/dev/null 2>&1 || true
  done
}
heartbeat &
HEARTBEAT_PID=$!
